...
/**
 * This is a callback that is able to receive multiple asynchronous responses
 * from the server, one per page. It feeds a blocking queue implemented by
 * {@link RowIteratorImpl}. Because of this blocking queue, it is important
 * that this callback uses a reserved connection, otherwise performance is
 * significantly degraded and becomes worse than the ordinary result callback
 * mechanism. Another big different with the existing RequestHandler is that
 * no retries should be attempted. Further, if the client interrupts the
 * request, a cancel message should be sent to the server.
 *
 * See CASSANDRA-11521 for more details.
 */
